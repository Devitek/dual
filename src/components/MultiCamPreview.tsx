...
export function MultiCamPreview({
  ...
  const pipGesture = useMemo(() => {
    const clampLeft = (left: number): number =>
      clamp(left, PIP_SIDE_MARGIN, screenW - PIP_SIDE_MARGIN - cur.current.width);
  ...
  return (
    <View style={StyleSheet.absoluteFill}>
      {mainPreview == null ? (
        ...
        <GestureDetector gesture={gesture}>
          <View style={StyleSheet.absoluteFill}>
            <NativePreviewView style={StyleSheet.absoluteFill} previewOutput={mainPreview} resizeMode="cover" />
          </View>
        </GestureDetector>
      )}
